...
public class OptionsMain {
	...
	public static void imprimeAyuda(Options opciones) {
		...
		ayuda.printHelp(
				Main.class.getCanonicalName()
						+ " [-a <asmfile>] [-h] [-i <infile>] [-m <mode>] [-o <outfile>] [-l <logfile>] ",
				opciones);
		System.exit(0);
	...
	static Options loadOptions() {
		...
		OptionBuilder
				.withDescription("Fichero donde se guarda la salida de la consola.");
		...
		while (contador < args.length) {
			...
			if (cmd.hasOption("a") && (!aSMtratado)) {
				...
				if ((posicionA < args.length)
						...
						&& (!args[posicionA].equalsIgnoreCase("-l"))
						&& (!args[posicionA].equalsIgnoreCase("--log"))) {
				...
				if ((posicionO < args.length)
						...
						&& (!args[posicionO].equalsIgnoreCase("-l"))
						&& (!args[posicionO].equalsIgnoreCase("--log"))) {
				...
				if ((posicionI < args.length)
						...
						&& (!args[posicionI].equalsIgnoreCase("-l"))
						&& (!args[posicionI].equalsIgnoreCase("--log"))) {
			...
			} else if (cmd.hasOption("l") && (!logTratado)) {
				posicionL++;
				contador++;
				if ((posicionL < args.length)
						&& (!args[posicionL].equalsIgnoreCase("-a"))
						&& (!args[posicionL].equalsIgnoreCase("--asm"))
						&& (!args[posicionL].equalsIgnoreCase("-o"))
						&& (!args[posicionL].equalsIgnoreCase("--out"))
						&& (!args[posicionL].equalsIgnoreCase("-i"))
						&& (!args[posicionL].equalsIgnoreCase("--in"))
						&& (!args[posicionL].equalsIgnoreCase("-l"))
						&& (!args[posicionL].equalsIgnoreCase("--log"))) {
					// si el argumento siguiente a "-i" es diferente a otro
					// argumento, sera un archivo.
					contador++;
					logTratado = true;
	...
	// METODO QUE CARGA LA OPCION L DE LOS ARGUMENTOS, Y PRUEBA A ABRIR EL
		// ARCHIVO PARA COMPROBAR SI EXISTE.
		public static boolean comprobarOpcionL(String[] args, CommandLine cmd,
				int posicionL, int contador) throws IOException {
			boolean archivo = false;
			if (cmd.hasOption("l")) {
				contador++;
				posicionL++;
				if (args[posicionL] != null) {
					contador++;
					try {
						// Probamos a abrir el archivo de O, si da error, no abrirar
						// y se mostrara excepcion.
